...
serve(async (req) => {
  ...
  console.log(`[${timestamp}] deals-import-export function called`);
    ...
    if (action === 'import') {
      ...
      for (const dealData of importDeals) {
        try {
          ...
          // If no ID match, check for duplicate by deal_name only
          if (!existingDeal) {
            console.log(`Checking for duplicate deal: "${dealData.deal_name}"`);
            const { data, error: duplicateError } = await supabaseClient
              .from('deals')
              .select('id, deal_name, stage')
