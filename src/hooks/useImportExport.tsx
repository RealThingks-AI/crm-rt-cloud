...
export const useImportExport = ({ moduleName, onRefresh, tableName = 'contacts_module' }: ImportExportOptions) => {
    ...
    const configs: Record<string, ColumnConfig> = {
      ...
      deals: {
        allowedColumns: [
          ...
          // Execution
          'supplier_portal_required', 'execution_started', 'begin_execution_date',
          // General
  ...
  const handleImport = async (file: File) => {
    try {
      ...
      if (tableName === 'deals') {
        console.log('DEALS IMPORT: Routing to handleDealsImport function');
  ...
  const handleExportAll = async (data: any[]) => {
    ...
    if (tableName === 'meetings') {
      try {
        ...
        exportToCSV(freshData || [], filename);
      } catch (error: any) {
  ...
  const exportToCSV = async (data: any[], filename: string) => {
    ...
    if (tableName === 'deals') {
      try {
        const { data: exportData, error } = await supabase.functions.invoke('deals-import-export-v2', {
          body: { action: 'export' }
        });
  ...
  const handleDealsImport = async (headers: string[], dataRows: string[][]) => {
    try {
      console.log(`Starting enhanced deals import with ${headers.length} headers and ${dataRows.length} rows`);
      console.log('CSV Headers:', headers);

      // Map headers to database columns with DIRECT MATCHING for exact roundtrip
      const mappedHeaders = headers.map(header => {
        const cleanHeader = header.trim();
        // First try exact match with allowed columns
        if (config.allowedColumns.includes(cleanHeader)) {
          return { original: header, mapped: cleanHeader };
        }
        // Then try fuzzy mapping
        return { original: header, mapped: mapHeader(header) };
      });

      console.log('Header mappings:', mappedHeaders.map(h => `${h.original} -> ${h.mapped}`));

      // Build deals array from CSV data with ALL FIELDS preserved
      const deals = dataRows.map((row, index) => {
        ...
        mappedHeaders.forEach((headerMap, headerIndex) => {
          if (headerMap.mapped && headerIndex < row.length) {
            ...
            // Preserve ALL values including empty strings for exact roundtrip
            if (rawValue !== undefined && rawValue !== null) {
              deal[headerMap.mapped] = rawValue;
            }
          }
        });

        // Ensure deal_name is present - CRITICAL requirement
        if (!deal.deal_name || deal.deal_name.trim() === '') {
          deal.deal_name = `Deal ${index + 1}`;
        }

        console.log(`Built deal ${index + 1}:`, {
          id: deal.id,
          deal_name: deal.deal_name,
          stage: deal.stage,
          fieldCount: Object.keys(deal).length
        });

        return deal;
      });

      console.log(`Sending ${deals.length} deals to edge function for import`);

      // Call enhanced edge function for perfect import
      const { data: result, error } = await supabase.functions.invoke('deals-import-export-v2', {
        body: { 
          action: 'import', 
          data: { 
            deals,
      ...
      let message = `Created: ${results.created}, Updated: ${results.updated}`;
      if (results.errors.length > 0) {
        message += `, Errors: ${results.errors.length}`;
      }
      
      toast({
        title: results.errors.length > 0 ? "Import Completed with Issues" : "Import Successful",
        description: message,
