...
const KanbanBoard = ({ deals, onRefresh, onEditDeal, onDeleteDeal, selectedDealIds = [], onDealSelection }: KanbanBoardProps) => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      // Apply probability logic based on target stage
      let probability = deal.probability;
      if (targetStage === 'Won') {
        probability = 100;
      } else if (targetStage === 'Lost' || targetStage === 'Dropped') {
        probability = 0;
      }
        ...
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 min-h-[600px] w-full ${
          ...
          {visibleStages.map((stage) => (
            <KanbanColumn
              ...
              onDealSelection={onDealSelection}
            />
          ))}
        </div>
