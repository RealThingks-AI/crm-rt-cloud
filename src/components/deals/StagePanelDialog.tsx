...
const StagePanelDialog = ({ open, onOpenChange, deal, onSuccess }: StagePanelDialogProps) => {
  ...
  const handleMoveToNextStage = async () => {
    ...
    try {
      ...
      console.log('Moving deal to next stage with data:', updateData);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {/* Deal Name - Editable for current stage */}
          {(deal.stage === 'Discussions' || deal.stage === 'Qualified' || deal.stage === 'RFQ' || deal.stage === 'Offered') && (
            <div>
              <h3 className="text-lg font-semibold mb-4">Deal Information</h3>
              <div>
                <Label htmlFor="deal_name" className="font-medium">Deal Name</Label>
                <Input
                  id="deal_name"
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold">Additional Deal Information</h3>
          ...
          <div className="flex justify-between items-center pt-4 border-t">
            ...
            <div className="flex space-x-3">
              ...
              {(deal.stage === 'Discussions' || deal.stage === 'Qualified' || deal.stage === 'RFQ') && (
                ...
                >
                  {isSubmitting ? 'Moving...' : `Move to ${getNextStage(deal.stage)}`}
                </Button>
              )}
              ...
              {deal.stage === 'Offered' && selectedFinalStage && (
                <Button 
                  ...
                  className="bg-blue-600 hover:bg-blue-700"
                >
                  {isSubmitting ? 'Moving...' : `Move to ${selectedFinalStage}`}
                </Button>
              )}
            </div>
          </div>
